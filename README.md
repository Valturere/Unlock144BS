...
6. Только на Xiaomi/HyperOS: в шаге **Фоновый автозапуск** нажмите **Открыть
   автозапуск**, включите переключатель напротив Unlock144BS и после возврата нажмите
   **Я включил автозапуск**. Без этого HyperOS может завершить Auto Fix после удаления
   приложения из списка недавних. Шаг отображается только при наличии Xiaomi
   SecurityCenter и открывает общий список автозапуска: надёжного API чтения состояния
   или перехода сразу к конкретному приложению в проверенной версии SecurityCenter нет.
7. На странице **О приложении** рекомендуется отключить настройку
...
8. При желании оставьте служебные уведомления отключёнными/скрытыми.
9. Запускайте Brawl Stars обычным способом.
